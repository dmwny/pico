...
function LoginContent() {
  ...
  const redirectPath = useMemo(
    () => normalizeRedirectPath(searchParams.get("redirect"), "/board"),
    [searchParams],
  );
  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-[var(--pico-cream)] px-4 py-10">
      ...
      >
        ...
        <h1 className="mt-8 font-serif text-5xl leading-none text-[var(--pico-dark)]">Welcome back</h1>
        <p className="mt-4 text-base leading-relaxed text-[var(--pico-body)]">
          Sign in and Pico will return you to <span className="font-semibold">{redirectPath}</span>.
        </p>
        ...
        <p className="mt-6 text-center text-sm text-[var(--pico-body)]">
          No account yet?{" "}
          <Link href={`/signup?redirect=${encodeURIComponent(redirectPath)}`} className="font-bold text-[var(--pico-orange)]">
            Sign up free
          </Link>
        </p>
      </div>
